...
#define TAG "de.appwerft.icystreammeta"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Icystreammeta_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Icystreammeta.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::IcystreammetaBindings::lookupGeneratedInit(
...
static void Icystreammeta_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Icystreammeta_getBinding)->GetFunction());
}

static void Icystreammeta_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::IcystreammetaBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry IcystreammetaBinding = {
	"de.appwerft.icystreammeta",
	Icystreammeta_init,
	Icystreammeta_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_de_appwerft_icystreammeta_IcystreammetaBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("de.appwerft.icystreammeta", &IcystreammetaBinding);
	titanium::KrollBindings::addExternalLookup(&(::IcystreammetaBindings::lookupGeneratedInit));
}
